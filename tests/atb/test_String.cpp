...
class AtbString : public ::testing::Test {
protected:
  void SetUp() override {
...
TEST_F(AtbString, CtorMoveSubStr) {
...
TEST_F(AtbString, CtorMove) {
  ...
  EXPECT_EQ(move.size, 0x4);
  EXPECT_EQ(move.capacity, 0x5);
}

TEST_F(AtbString, ReserveDelete) {
...
TEST_F(AtbString, ResizeAndFill) {
...
TEST_F(AtbString, IsEqualToSubStr) {
  constexpr auto expected_str = std::string_view{"Coucou"};
  constexpr auto other_str = std::string_view{"Chocolatine"};
